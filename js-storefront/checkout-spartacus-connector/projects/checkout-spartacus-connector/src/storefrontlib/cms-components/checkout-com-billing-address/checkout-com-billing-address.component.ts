...
@Component({
  ...
  templateUrl: './checkout-com-billing-address.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class CheckoutComBillingAddressComponent implements OnInit {
  ...
  getRegionBindingLabel(regions: Region[]) {
    if (regions?.length) {
      if (regions[0].name) {
        return 'name';
      }
  ...
  getRegionBindingValue(regions: Region[]) {
    if (regions?.length) {
